  ...
  return new Promise<boolean>((resolve, reject) => {
    ...
    ftps.ls().exec((error, result) => {
      if (!!error) {
        ...
        if (!!result.error) {
          // everything went ok (request/answer) but the server has returned an error
          reject(result.error);
        }
        resolve(true);
      }
...
function parseLsResult(result: string): LsResult[] {
  ...
  columns.forEach((line) => {
    if (line.length !== 9) {
      throw new Error(
        `LS Parse : the following data could not be parsed as an LS result:\n${result}`
      );
    }
  });
...
export async function getReceipts() {
      ...
      .exec((error, result) => {
        if (!!error) {
          ...
          if (!result.data) {
            ...
            resolve(
              list.filter((result) => result.isFile).map((file) => file.name)
            );
          }
        }
      });
  ...
  const downloadedFileNames = await Promise.all(
    fileNames.map((fileName) => {
      return new Promise<string>((resolve) => {
        // 'get' the current file (download to the given folder) then execute the callback
        ftps
          .get(`${remoteFolder}/${fileName}`, `${localFolder}/${fileName}`)
          .exec((error, result) => {
            if (!!error || !!result.error) {
              ...
              console.warn(
                `An error as prevent ${fileName} to be correctly downloaded!`
              );
 ...
 */
export async function sendPayment(fileNames: string[]) {
  // TODO TEST THIS FUNCTION WITH 1 AND MORE FILES !!!
  const ftps = getFtps();
    ...
    fileNames.map(
      (fileName) =>
        new Promise<string>((resolve) => {
          ...
          ftps
            .put(`${localFolder}/${fileName}`, `${remoteFolder}/${fileName}`)
            .exec((error, result) => {
              if (!!error || !!result.error) {
                ...
                console.warn(
                  `An error as prevent ${fileName} to be correctly uploaded!!`
                );
                console.warn(error, result.error);
                resolve("");
              } else {
